...
startOptionValueList:
	...
	| PASSWORD_SYMBOL (FOR_SYMBOL user)? TO_SYMBOL RANDOM_SYMBOL replacePassword?
		retainCurrentPassword?;
...
showStatement:
	SHOW_SYMBOL (
		...
		| showCommandType? value = COLUMNS_SYMBOL (
			FROM_SYMBOL
			| IN_SYMBOL
		...
		| value = PROFILE_SYMBOL (
			profileType (COMMA_SYMBOL profileType)*
...
nonBlocking: NONBLOCKING_SYMBOL? | /* empty */;
...
resourceGroupVcpuList:
	VCPU_SYMBOL equal? vcpuNumOrRange (
		COMMA_SYMBOL? vcpuNumOrRange
	)*;
...
alterResourceGroup:
	ALTER_SYMBOL RESOURCE_SYMBOL GROUP_SYMBOL resourceGroupRef resourceGroupVcpuList?
		resourceGroupPriority? resourceGroupEnableDisable? FORCE_SYMBOL?;

setResourceGroup:
	SET_SYMBOL RESOURCE_SYMBOL GROUP_SYMBOL identifier (
		FOR_SYMBOL threadIdList
	)?;
...
simpleExpr:
	...
	| MATCH_SYMBOL identListArg AGAINST_SYMBOL OPEN_PAR_SYMBOL bitExpr fulltextOptions?
		CLOSE_PAR_SYMBOL																# simpleExprMatch
...
sumExpr:
	...
	| name = (BIT_AND_SYMBOL | BIT_OR_SYMBOL | BIT_XOR_SYMBOL) OPEN_PAR_SYMBOL inSumExpr
...
windowFunctionCall:
	...
	| (LEAD_SYMBOL | LAG_SYMBOL) OPEN_PAR_SYMBOL expr leadLagInfo? CLOSE_PAR_SYMBOL nullTreatment?
		windowingClause
...
leadLagInfo:
	COMMA_SYMBOL (ulonglong_number | PARAM_MARKER) (
		COMMA_SYMBOL expr
	)?;
...
jsonFunction:
	...
	| JSON_OBJECTAGG_SYMBOL OPEN_PAR_SYMBOL inSumExpr COMMA_SYMBOL inSumExpr CLOSE_PAR_SYMBOL
		windowingClause?;
...
runtimeFunctionCall:
	...
	name = CHAR_SYMBOL OPEN_PAR_SYMBOL exprList (
		USING_SYMBOL charsetName
	)? CLOSE_PAR_SYMBOL
	...
	| name = INSERT_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr COMMA_SYMBOL expr COMMA_SYMBOL
		expr CLOSE_PAR_SYMBOL
	| name = INTERVAL_SYMBOL OPEN_PAR_SYMBOL expr (
		COMMA_SYMBOL expr
	)+ CLOSE_PAR_SYMBOL
	...
	| name = TIMESTAMP_SYMBOL OPEN_PAR_SYMBOL expr (
		COMMA_SYMBOL expr
	)? CLOSE_PAR_SYMBOL
	| trimFunction
	...
	| name = (DATE_ADD_SYMBOL | DATE_SUB_SYMBOL) OPEN_PAR_SYMBOL expr COMMA_SYMBOL INTERVAL_SYMBOL
		expr interval CLOSE_PAR_SYMBOL
	...
	| name = (TIMESTAMP_ADD_SYMBOL | TIMESTAMP_DIFF_SYMBOL) OPEN_PAR_SYMBOL intervalTimeStamp
		COMMA_SYMBOL expr COMMA_SYMBOL expr CLOSE_PAR_SYMBOL
	...
	| name = FORMAT_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr (
		COMMA_SYMBOL expr
	)? CLOSE_PAR_SYMBOL
	...
	| name = REPLACE_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr COMMA_SYMBOL expr
		CLOSE_PAR_SYMBOL
...
weightStringLevels:
	LEVEL_SYMBOL (
		...
		| weightStringLevelListItem (
			COMMA_SYMBOL weightStringLevelListItem
		)*
	);
...
searchJsonFunction:
	JSON_VALUE_SYMBOL OPEN_PAR_SYMBOL (
		expr COMMA_SYMBOL textLiteral jsonValueReturning? jsonValueOnEmpty? jsonValueOnError?
	) CLOSE_PAR_SYMBOL;

jsonValueReturning: RETURNING_SYMBOL castType;
...
keyListWithExpression:
	OPEN_PAR_SYMBOL keyPartOrExpression (
		COMMA_SYMBOL keyPartOrExpression
	)* CLOSE_PAR_SYMBOL;
...
nchar: type = NCHAR_SYMBOL | type = NATIONAL_SYMBOL CHAR_SYMBOL;
...
createTableOption: // In the order as they appear in the server grammar.
	...
	| option = TABLESPACE_SYMBOL (EQUAL_OPERATOR? | /* empty */) identifier
...
partitionTypeDef:
	LINEAR_SYMBOL? KEY_SYMBOL partitionKeyAlgorithm? OPEN_PAR_SYMBOL identifierList?
		CLOSE_PAR_SYMBOL													# partitionDefKey
...
partitionDefinitions:
	OPEN_PAR_SYMBOL partitionDefinition (
		COMMA_SYMBOL partitionDefinition
	)* CLOSE_PAR_SYMBOL;
...
partitionDefinition:
	PARTITION_SYMBOL identifier (
		...
		OPEN_PAR_SYMBOL subpartitionDefinition (
			COMMA_SYMBOL subpartitionDefinition
		)* CLOSE_PAR_SYMBOL
	)?;
...
partitionValueItemListParen:
	OPEN_PAR_SYMBOL partitionValueItem (
		COMMA_SYMBOL partitionValueItem
	)* CLOSE_PAR_SYMBOL;
...
// For each object we have at least 2 rules here: 1) The name when creating that object. 2) The name
// when used to reference it from other rules.
// 
// Sometimes we need additional reference rules with different form, depending on the place such a
// reference is used.
...
columnInternalRefList: // column_list (+ parentheses) + opt_derived_column_list in sql_yacc.yy
	OPEN_PAR_SYMBOL columnInternalRef (
		COMMA_SYMBOL columnInternalRef
	)* CLOSE_PAR_SYMBOL;
...
// TEXT_STRING_sys + TEXT_STRING_literal + TEXT_STRING_filesystem + TEXT_STRING +
// TEXT_STRING_password + TEXT_STRING_validated in sql_yacc.yy.
textStringLiteral:
...
// A special variant of a text string that must not contain a linebreak (TEXT_STRING_sys_nonewline
// in sql_yacc.yy). Check validity in semantic phase.
...
// Note: rules for non-reserved keywords have changed significantly with MySQL 8.0.17, which make
// their version dependent handling complicated. Comments for keyword rules are taken over directly
// from the server grammar, but usually don't apply here since we don't have something like
// shift/reduce conflicts in ANTLR4 (which those ugly rules try to overcome).

// Non-reserved keywords are allowed as unquoted identifiers in general.
// 
// OTOH, in a few particular cases statement-specific rules are used instead of `ident_keyword` to
// avoid grammar ambiguities:
// 
// * `label_keyword` for SP label names * `role_keyword` for role names * `lvalue_keyword` for
// variable prefixes and names in left sides of assignments in SET statements
// 
// Normally, new non-reserved words should be added to the the rule `ident_keywords_unambiguous`. If
// they cause grammar conflicts, try one of `ident_keywords_ambiguous_...` rules instead.
identifierKeyword:
	(
		labelKeyword
...
// Keywords that we allow for labels in SPs in the unquoted form. Any keyword that is allowed to
// begin a statement or routine characteristics must be in `ident_keywords_ambiguous_2_labels`
// above, otherwise we get (harmful) shift/reduce conflicts.
// 
// Not allowed:
// 
// ident_keywords_ambiguous_1_roles_and_labels ident_keywords_ambiguous_2_labels
labelKeyword:
	(
		roleOrLabelKeyword
		| EVENT_SYMBOL
...
// These are the non-reserved keywords which may be used for unquoted identifiers everywhere without
// introducing grammar conflicts:
identifierKeywordsUnambiguous:
	(
		ACTION_SYMBOL
		| ACCOUNT_SYMBOL
...
// Not allowed:
// 
// ident_keywords_ambiguous_1_roles_and_labels ident_keywords_ambiguous_3_roles
roleKeyword:
...
// Non-reserved words allowed for unquoted unprefixed variable names and unquoted variable prefixes
// in the left side of assignments in SET statements:
// 
// Not allowed:
// 
...
// These non-reserved keywords cannot be used as unquoted unprefixed variable names and unquoted
// variable prefixes in the left side of assignments in SET statements:
identifierKeywordsAmbiguous4SystemVariables:
	GLOBAL_SYMBOL
	| LOCAL_SYMBOL
	| PERSIST_SYMBOL
...
// These are the non-reserved keywords which may be used for roles or idents. Keywords defined only
// for specific server versions are handled at lexer level and so cannot match this rule if the
// current server version doesn't allow them. Hence we don't need predicates here for them.
roleOrIdentifierKeyword:
	(
...
roleOrLabelKeyword:
	(
		...
		| CPU_SYMBOL
		/*
		 Although a reserved keyword in SQL:2003 (and :2008), not reserved in MySQL per WL#2111
		 specification.
		 */
		| CURRENT_SYMBOL
		| CURSOR_NAME_SYMBOL
	...
	// Tokens that entered or left this rule in specific versions and are not automatically handled
	// in the lexer.
	| SHUTDOWN_SYMBOL
	| (
		CUBE_SYMBOL
